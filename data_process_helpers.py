...
def read_files(files_names, files_addr, headers, dtypes):
    ...
    for name in files_names:
        df = pd.read_csv(files_addr + name, sep=",",
                         names=headers, dtype=dtypes, header=None)
        df["datetime"] = df["date"] + " " + df["time"]
        df["datetime"] = pd.to_datetime(
            df["datetime"], format="%m/%d/%Y %H:%M")
...
def modify_time_inteval(dfs, time_slice, time_inteval):
    for i in range(len(dfs)):
        ...
        headers = ["open_price", "highest_price", "lowest_price",
                   "close_price", "volumn", "datetime", "hour", "minute"]
        ...
        while cur_time < end_time:
            ...
            while target_time < cur_time and j < row_num:
                ...
                # k = j
                if target_time < prev_time or target_time > cur_time:
                    continue
                have_updates = True
                cur_open_price = dfs[i].iloc[k]["open_price"] if is_first else cur_open_price
                is_first = False
                cur_close_price = dfs[i].iloc[k]["close_price"]
                cur_highest_price = dfs[i].iloc[k]["highest_price"] if cur_highest_price < dfs[
                    i].iloc[k]["highest_price"] else cur_highest_price
                cur_lowest_price = dfs[i].iloc[k]["lowest_price"] if cur_lowest_price > dfs[
                    i].iloc[k]["lowest_price"] else cur_lowest_price
